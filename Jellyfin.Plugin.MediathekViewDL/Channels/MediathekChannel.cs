...
public class MediathekChannel : IChannel, IRequiresMediaInfoCallback
{
    ...
    /// <inheritdoc />
    // Distinct from upstream's own "Mediathek (Virtual)" channel name (same convention as the
    // plugin's own display name "CuriousApes-MediathekView-Downloader" and the HttpClient/XML
    // collision fixes elsewhere in this fork): if a user ever runs both this fork and the
    // original plugin side by side, Jellyfin's channel manager should not be able to confuse the
    // two channels with each other just because they share the same display name.
    public string Name => "Mediathek (Virtual) [CuriousApe2020 Fork]";
    ...
    /// <inheritdoc />
    public string HomePageUrl => "https://github.com/CuriousApe2020/jellyfin-plugin-MediathekViewDL";
    ...
    {
        // The channel is registered unconditionally at plugin startup (Jellyfin's channel system has
        // no concept of "register only if configured"), so without this check it would show up in
        // every installation's channel list even for users who never touch virtual subscriptions -
        // IChannel instances are DI singletons with no per-request re-evaluation of whether they're
        // wanted otherwise. Hiding it here until at least one subscription is actually marked
        // "Virtuell" keeps it out of sight for everyone else.
        var config = _configurationProvider.ConfigurationOrNull;
