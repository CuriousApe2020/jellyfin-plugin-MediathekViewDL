...
public class FFmpegDownloadHandler : IDownloadHandler
{
    ...
    public async Task<bool> ExecuteAsync(DownloadItem item, DownloadJob job, IProgress<double> progress, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var res = await _ffmpegService.DownloadFileAsync(item.SourceUrl, tempPath, readRate, progress, cancellationToken, audioLanguageCode, item.CleanAudioTrackLabel, job.MediaMetadata).ConfigureAwait(false);
            if (!res)
            {
                return false;
            }
